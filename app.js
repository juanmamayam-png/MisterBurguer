/* ═══════════════════════════════════════════════
   MISTER BURGER POS · app.js  v4
   "Base de datos" simulada en localStorage con
   estructura relacional completa:
     · users  · products  · tables  · orders
     · order_items  · work_days  · transactions
═══════════════════════════════════════════════ */
'use strict';

/* ─────────────────────────────────────────────
   DB — estructura completa en localStorage
───────────────────────────────────────────── */
const DB_KEYS = {
  users:      'mb4_users',
  products:   'mb4_products',
  tables:     'mb4_tables',
  orders:     'mb4_orders',
  items:      'mb4_items',
  days:       'mb4_days',
  trans:      'mb4_transactions',
  seq:        'mb4_sequences',
  session:    'mb4_session',
};
...
const DEFAULT_PRODUCTS = [
  ...
  {id:4, name:'Vaquera',cost:9500,price:21500,cat:'Hamburguesas',emoji:'🤠',image:null,status:'active',desc:'Pan artesanal, queso, 150g carne 100% res, costilla ahumada a la plancha, tomate, cebolla y lechuga'},
  {id:5, name:'Campesina',cost:10000,price:22000,cat:'Hamburguesas',emoji:'🌽',image:null,status:'active',desc:'Pan artesanal, queso, 150g res maduro, huevo frito, chorizo, tocineta ahumada, maíz tierno, tomate, cebolla y lechuga'},
  {id:6, name:'Criolla',cost:9000,price:20500,cat:'Hamburguesas',emoji:'🥚',image:null,status:'active',desc:'Pan artesanal, queso, 150g carne 100% res, tocineta ahumada, huevo frito, maíz tierno, tomate, cebolla y lechuga'},
  ...
  {id:10,name:'Callejera',cost:9000,price:20000,cat:'Hamburguesas',emoji:'🌆',image:null,status:'active',desc:'Pan artesanal, doble queso, 150g carne 100% res, tocineta ahumada, cebolla con salsa rosada, ripio de papa'},
  ...
  {id:12,name:'Tentación',cost:9500,price:21000,cat:'Hamburguesas',emoji:'🍌',image:null,status:'active',desc:'Plátano maduro, doble queso, tocineta, carne 150g, huevo frito, tomate, cebolla, lechuga y salsa dulce maíz'},
  ...
  {id:14,name:'Marinera',cost:10000,price:21500,cat:'Hamburguesas',emoji:'🦐',image:null,status:'active',desc:'Pan artesanal, queso, 150g carne 100% res, camarones salteados con salsa especial, tomate, cebolla y lechuga'},
  ...
  {id:22,name:'Pechuga a la Plancha',cost:16000,price:35000,cat:'Especiales',emoji:'🍖',image:null,status:'active',desc:'Acompañamiento de papas a la francesa, ensalada. 2 opciones para el plato'},
  ...
  {id:24,name:'Patacón Especial',cost:14000,price:32000,cat:'Especiales',emoji:'🫔',image:null,status:'active',desc:'Lomo fino de res, pechuga, lomo de cerdo, chorizo, patacón, maíz tierno, queso gratinado, ripio de papa y huevos de codorniz'},
  {id:25,name:'Maicito',cost:14000,price:32000,cat:'Especiales',emoji:'🌽',image:null,status:'active',desc:'Carne mixta, pollo, lomo fino de res, lomo de cerdo, cebolla grillé, papas a la francesa, 2 yucas, queso gratinado, maíz tierno'},
  ...
  {id:41,name:'Limonada de Coco',cost:4000,price:11000,cat:'Bebidas',emoji:'🥥',image:null,status:'active',desc:'Refrescante con coco'},
  ...
  {id:52,name:'Patacones con Hogao',cost:2500,price:7000,cat:'Entradas',emoji:'🫓',image:null,status:'active',desc:'Patacones con hogao casero'},
  {id:53,name:'Arepas con Hogao',cost:2500,price:7000,cat:'Entradas',emoji:'🫓',image:null,status:'active',desc:'Arepas tradicionales con hogao'},
  {id:54,name:'Papas Criollas',cost:2500,price:7000,cat:'Entradas',emoji:'🥔',image:null,status:'active',desc:'Papas criollas fritas'},
  {id:55,name:'Aritos de Cebolla',cost:3000,price:8000,cat:'Entradas',emoji:'🧅',image:null,status:'active',desc:'Anillos de cebolla fritos'},
...
const DB = {
  ...
  set(key,val){ try{ localStorage.setItem(key,JSON.stringify(val)); }catch(e){ console.warn('[MB4] ls error',e); } },
  seq(key){ const s=this.get(DB_KEYS.seq)||{}; s[key]=(s[key]||0)+1; this.set(DB_KEYS.seq,s); return s[key]; },
  ...
  saveUsers(arr){ this.set(DB_KEYS.users,arr); },

  /* products */
  getProducts(){ return this.get(DB_KEYS.products)||DEFAULT_PRODUCTS; },
  saveProducts(arr){ this.set(DB_KEYS.products,arr); },
  upsertProduct(p){
    const arr=this.getProducts();
    const i=arr.findIndex(x=>x.id===p.id);
    if(i>=0) arr[i]={...arr[i],...p}; else arr.push({id:this.seq('product'),...p});
  ...
  getTables(){
    ...
    if(!t||t.length<37||t[0].floor===undefined){
      const fresh=buildTables();
      if(t) t.forEach(o=>{ const x=fresh.find(f=>f.id===o.id); if(x)x.status=o.status; });
      this.saveTables(fresh); return fresh;
    }
    return t;
  },
  saveTables(arr){ this.set(DB_KEYS.tables,arr); },
  updateTableStatus(id,status){ const t=this.getTables(); const x=t.find(x=>x.id===id); if(x){x.status=status;this.saveTables(t);} },

  /* orders */
  getOrders(){ return this.get(DB_KEYS.orders)||[]; },
  saveOrders(arr){ this.set(DB_KEYS.orders,arr); },
  createOrder(o){ const arr=this.getOrders(); const id=this.seq('order'); arr.push({id,...o}); this.saveOrders(arr); return id; },
  updateOrder(id,patch){ const arr=this.getOrders(); const i=arr.findIndex(x=>x.id===id); if(i>=0)arr[i]={...arr[i],...patch}; this.saveOrders(arr); },
  getActiveOrder(tableId){ return this.getOrders().find(o=>o.tableId===tableId&&(o.status==='active'||o.status==='pending')); },

  /* order items */
  getItems(){ return this.get(DB_KEYS.items)||[]; },
  saveItems(arr){ this.set(DB_KEYS.items,arr); },
  addItem(it){ const arr=this.getItems(); const id=this.seq('item'); arr.push({id,...it}); this.saveItems(arr); return id; },
  updateItem(id,patch){ const arr=this.getItems(); const i=arr.findIndex(x=>x.id===id); if(i>=0)arr[i]={...arr[i],...patch}; this.saveItems(arr); },
  orderItems(oid){ return this.getItems().filter(i=>i.orderId===oid); },

  /* work days */
  getDays(){ return this.get(DB_KEYS.days)||[]; },
  saveDays(arr){ this.set(DB_KEYS.days,arr); },
  getCurrentDay(){ const days=this.getDays(); return days.find(d=>d.status==='open')||null; },
  createDay(d){ const arr=this.getDays(); const id=this.seq('day'); arr.push({id,...d}); this.saveDays(arr); return id; },
  updateDay(id,patch){ const arr=this.getDays(); const i=arr.findIndex(x=>x.id===id); if(i>=0)arr[i]={...arr[i],...patch}; this.saveDays(arr); },

  /* transactions — registro contable */
  getTransactions(){ return this.get(DB_KEYS.trans)||[]; },
  addTransaction(t){ const arr=this.getTransactions(); const id=this.seq('trans'); arr.push({id,...t}); this.set(DB_KEYS.trans,arr); return id; },
  dayTransactions(dayId){ return this.getTransactions().filter(t=>t.dayId===dayId); },
...
function orderTotal(oid){
  return DB.orderItems(oid).filter(i=>i.status==='active').reduce((s,i)=>s+i.price*i.qty,0);
}
function orderCost(oid){
  return DB.orderItems(oid).filter(i=>i.status==='active').reduce((s,i)=>s+(i.cost||0)*i.qty,0);
}
...
function showApp(which){
  ['client','staff','login'].forEach(a=>{ const el=$(`app-${a}`); if(el) el.classList.toggle('hidden',a!==which); });
}
function showLogin(){ showApp('login'); }
function backToClient(){ showApp('client'); }
...
function initApp(){
  ...
  if(saved){ S.user=saved; bootStaff(); }
  else{ showApp('client'); renderClientMenu(); }
  updateLocalBadges();
}
...
function bootStaff(){
  ...
  updateLocalBadges();
  // Default section
  if(S.user.role==='boss') staffSection('dashboard');
...
/* ─────────────────────────────────────────────
   STAFF NAV — sidebar dinámico
───────────────────────────────────────────── */
const WAITER_NAV = [
  { label:'Operación' },
  { id:'tables',  icon:'fa-table-cells',    text:'Mesas',         action:()=>staffSection('tables') },
  { id:'order',   icon:'fa-receipt',         text:'Pedido Activo', action:()=>staffSection('order') },
  { label:'Menú' },
  { id:'menuview',icon:'fa-utensils',        text:'Ver Menú',      action:()=>staffSection('menuview') },
];

const BOSS_NAV = [
  { label:'Dashboard' },
  { id:'dashboard',icon:'fa-chart-pie',     text:'Resumen',             action:()=>staffSection('dashboard') },
  { label:'Operación' },
  { id:'tables',   icon:'fa-table-cells',   text:'Todas las Mesas',     action:()=>staffSection('tables') },
  { id:'orders',   icon:'fa-list-check',    text:'Todos los Pedidos',   action:()=>staffSection('orders') },
  { label:'Administración' },
  { id:'products', icon:'fa-box',           text:'Productos',           action:()=>staffSection('products') },
  { id:'users',    icon:'fa-users',         text:'Usuarios',            action:()=>staffSection('users') },
  { id:'reports',  icon:'fa-chart-line',    text:'Reporte Diario',      action:()=>staffSection('reports') },
  { label:'Local', isBottom:true },
  { id:'local',    icon:'fa-store',         text:'Abrir/Cerrar Local',  action:()=>toggleLocal(), dynamic:true },
];
...
function renderStaffNav(){
  ...
  nav.innerHTML = items.map(item=>{
    ...
    return `<button class="st-nav__item${isLocal?' local-'+( DB.getCurrentDay()?'open':'close'):''}" id="nav-${item.id}" onclick="navClick('${item.id}')">
      <i class="fa-solid ${item.icon}"></i><span>${isLocal && DB.getCurrentDay()?'Cerrar Local':item.text}</span>
    </button>`;
  }).join('');
}
...
function navClick(id){
  const map = {
    dashboard:()=>staffSection('dashboard'),
    tables:   ()=>staffSection('tables'),
    order:    ()=>staffSection('order'),
    orders:   ()=>staffSection('orders'),
    menuview: ()=>staffSection('menuview'),
    products: ()=>staffSection('products'),
    users:    ()=>staffSection('users'),
    reports:  ()=>staffSection('reports'),
    local:    ()=>toggleLocal(),
  };
  if(map[id]) map[id]();
}
...
function renderStaffProfile(){
  ...
  const u=S.user;
  p.innerHTML=`
    <div class="sp-av">${u.name[0]}</div>
    <div><div class="sp-name">${u.name}</div>
    <div class="sp-role">${u.role==='boss'?'Jefe / Admin':'Mesero'}</div></div>`;
}

function updateStaffUser(){
  const el=$('st-user'); if(!el||!S.user) return;
  el.innerHTML=`<div class="av">${S.user.name[0]}</div><span>${S.user.name}</span><span class="badge ${S.user.role==='boss'?'badge-acc':'badge-blue'}">${S.user.role==='boss'?'Jefe':'Mesero'}</span>`;
}
...
function toggleSidebar(){
  ...
  if(window.innerWidth<=900){
    ...
    sc.classList.toggle('expanded',sidebarCollapsed);
  }
}
  ...
  if(prods.length){
    ...
    html+=prods.map(p=>`
      <div class="ss-item" onmousedown="ssPickProduct(${p.id})">
        <span class="ss-em">${p.image?`<img src="${p.image}" style="width:24px;height:24px;object-fit:cover;border-radius:4px">`:p.emoji}</span>
        <div class="ss-txt"><div class="ss-name">${p.name}</div><div class="ss-sub">${p.cat}</div></div>
  ...
  if(tables.length){
    ...
    html+=tables.map(t=>{
      ...
      return `<div class="ss-item" onmousedown="ssPickTable(${t.id})">
        ...
        <div class="ss-txt"><div class="ss-name">Mesa ${t.num} · Piso ${t.floor}</div><div class="ss-sub">${{free:'Libre',occupied:'Ocupada',pending:'Pago pendiente'}[t.status]||t.status}${o?' · '+fmtCOP(orderTotal(o.id)):''}</div></div>
      </div>`;
    }).join('');
  }
...
function ssPickProduct(pid){
  ...
  if(S.activeOrder) openAdd(pid);
  else toast('Selecciona una mesa para agregar productos','info');
}
function ssPickTable(tid){
  $('st-search').value=''; $('ss-drop').classList.add('hidden');
  S.floor=DB.getTables().find(t=>t.id===tid)?.floor||1;
  if(S.user.role==='boss'){ staffSection('tables'); bossPickTable(tid); }
  else { staffSection('tables'); waiterPickTable(tid); }
}

/* ─────────────────────────────────────────────
   LOCAL STATUS (open/closed)
───────────────────────────────────────────── */
function updateLocalBadges(){
  ...
  // Refresh nav local button
  const lb=$('nav-local'); if(lb){ lb.className=`st-nav__item ${open?'local-open':'local-close'}`; lb.querySelector('span').textContent=open?'Cerrar Local':'Abrir Local'; }
}

function toggleLocal(){
  if(DB.getCurrentDay()) openCloseDayModal();
  else                    openOpenModal();
}
...
function clientNav(page,btn){
  ...
  else { document.querySelectorAll('.cn__link').forEach(b=>{ if(b.getAttribute('onclick')?.includes(page)) b.classList.add('active'); }); }
  if(page==='menu') renderClientMenu();
}

function toggleMob(){
  const m=$('mob-menu'); if(m) m.classList.toggle('hidden');
}
...
function switchFloor(f,ctx){ S.floor=f; staffSection(ctx==='boss'?'tables':ctx); }
...
function renderTables(c){
  ...
  c.innerHTML=`
    <div class="ph-row">
      <div class="ph"><h2>${isBoss?'Todas las Mesas':'Mis Mesas'}</h2><p>Piso ${S.floor} · 🟢 Libre · 🔴 Ocupada · 💛 Pago pendiente</p></div>
    </div>
    ${floorTabsHTML(isBoss?'boss':'tables')}
    ...
    <div class="tables-grid">
      ${tables.map(t=>{
        ...
        const statusTxt=t.status==='pending'?'Pago pendiente':t.status==='occupied'?(mine&&!isBoss?'Mi pedido':'Ocupada'):'Libre';
        return `<div class="table-card ${t.status} ${S.selectedTable===t.id?'selected':''}" onclick="${isBoss?`bossPickTable(${t.id})`:`waiterPickTable(${t.id})`}">
    ...
    ${S.selectedTable&&isBoss?`<div id="inline-order" style="margin-top:28px"></div>`:''}`;

  if(S.selectedTable&&isBoss){
    const o=DB.getActiveOrder(S.selectedTable);
    S.activeOrder=o||null;
    renderOrderPanel($('inline-order'),true);
  }
}

function waiterPickTable(tid){
  ...
  S.selectedTable=tid; S.activeOrder=o||null;
  const t=DB.getTables().find(x=>x.id===tid);
  toast(`Mesa ${t?.num} seleccionada`,'info');
  staffSection('order');
}
function bossPickTable(tid){
  S.selectedTable=tid;
  const o=DB.getActiveOrder(tid);
  S.activeOrder=o||null;
  renderTables($('st-content'));
}
...
function renderOrderSection(c){
  if(!S.selectedTable){ c.innerHTML=`<div class="ph"><h2>Pedido Activo</h2></div><div style="display:flex;align-items:center;justify-content:center;height:200px;color:var(--text-m);font-size:15px;gap:10px"><i class="fa-solid fa-table-cells"></i> Selecciona primero una mesa</div>`; return; }
  renderOrderPanel(c,S.user.role==='boss');
}

function renderOrderPanel(container,isBoss){
  const t=DB.getTables().find(x=>x.id===S.selectedTable);
  const o=S.activeOrder;
  const isPending=o&&o.status==='pending';

  container.innerHTML=`
    <div class="ph-row">
      <div class="ph"><h2>Mesa ${t?.num||'?'} · Piso ${t?.floor||'?'}</h2>
        <p>${o?`Pedido #${o.id}`:'Sin pedido'} ${isPending?`<span class="badge badge-yellow">🔒 Pago pendiente</span>`:''}</p>
      </div>
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ${!o?`<button class="pill-btn pill-btn--green" onclick="createOrder(${S.selectedTable})"><i class="fa-solid fa-plus"></i> Nuevo Pedido</button>`:''}
        ${isBoss&&o?`<button class="pill-btn pill-btn--blue" onclick="openMoveModal()"><i class="fa-solid fa-arrows-alt"></i> Cambiar Mesa</button>`:''}
        ${o&&!isPending?`<button class="pill-btn pill-btn--danger" onclick="requestPay(${o.id})"><i class="fa-solid fa-money-bill-wave"></i> Solicitar Cobro</button>`:''}
        ${isBoss&&isPending?`<button class="pill-btn pill-btn--green" onclick="openPayModal(${o.id})"><i class="fa-solid fa-circle-check"></i> Cobrar y Liberar</button>`:''}
        <button class="pill-btn" onclick="staffSection('${isBoss?'tables':'tables'}')"><i class="fa-solid fa-arrow-left"></i> Volver</button>
      </div>
    </div>
    <div class="order-layout">
      <div class="order-prods">
        <div class="order-search-bar">
          <input class="field-c" id="op-search" placeholder="🔍 Buscar producto…" oninput="renderOrderProds()" ${!o||isPending?'disabled':''} style="flex:1">
          <select class="field-c" id="op-cat" style="max-width:155px" onchange="renderOrderProds()" ${!o||isPending?'disabled':''}>
            <option value="">Todas</option><option>Hamburguesas</option><option>Especiales</option><option>Hot Dog</option><option>Bebidas</option><option>Infantil</option><option>Entradas</option>
          </select>
        </div>
        ${isPending?`<div class="locked-state"><i class="fa-solid fa-lock"></i><h4>Pedido bloqueado</h4><p>Esperando confirmación de pago por el Jefe</p></div>`:''}
        <div class="order-prods-grid" id="op-grid"></div>
      </div>
      <div class="cart-panel">
        <div class="cart-head"><h3><i class="fa-solid fa-receipt"></i> Pedido</h3>
...
function renderOrderProds(){
  ...
  grid.innerHTML=prods.map(p=>p.image?`
    <div class="opc with-img" onclick="openAdd(${p.id})">
      ...
      <div class="oi"><div class="on">${p.name}</div><div class="oc">${p.cat}</div><div class="op">${fmtCOP(p.price)}</div></div>
    </div>`:`
    <div class="opc" onclick="openAdd(${p.id})">
      <span class="oe">${p.emoji}</span>
      <span class="on">${p.name}</span>
      <span class="oc">${p.cat}</span>
      <span class="op">${fmtCOP(p.price)}</span>
    </div>`).join('');
}
...
function renderCartItems(oid,locked){
  ...
  return items.map(it=>{
    ...
    return `<div class="ci ${c?'cancelled':''}" id="ci-${it.id}">
      ${p?.image?`<img src="${p.image}" style="width:28px;height:28px;border-radius:5px;object-fit:cover;flex-shrink:0">`:
        `<span class="ci-em">${p?.emoji||'🍔'}</span>`}
      <div class="ci-info">
...
function renderCartFoot(oid){
  ...
  return `
    <div class="cr"><span>Productos activos</span><span>${active.length}</span></div>
...
function createOrder(tableId){
  ...
  renderOrderPanel($('st-content').contains(document.querySelector('.order-layout'))?$('st-content'):$('inline-order')||$('st-content'),S.user.role==='boss');
}
...
function openPayModal(oid){
  ...
  const total=orderTotal(oid);
  $('pay-sum').innerHTML=`
    <div class="pt-title">Mesa ${t?.num} · Piso ${t?.floor} · Pedido #${oid}</div>
    ${items.map(i=>{ const p=DB.getProducts().find(x=>x.id===i.productId);
      return `<div class="pi ${i.status==='cancelled'?'cancelled':''}"><span>${p?.emoji||''} ${p?.name||'?'} ×${i.qty}</span><span>${fmtCOP(i.price*i.qty)}</span></div>`;
    }).join('')}
...
function confirmPay(){
  ...
  const day=DB.getCurrentDay();
  if(day){
    DB.addTransaction({ type:'income', dayId:day.id, orderId:oid, amount:total, cost, profit:total-cost, method, description:`Pedido #${oid} Mesa ${t?.num} P${t?.floor}`, createdAt:new Date().toISOString() });
  }
...
function openMoveModal(){
  S.moveTarget=null;
  const fl1=DB.getTables().filter(t=>t.floor===1&&t.id!==S.selectedTable);
  const fl2=DB.getTables().filter(t=>t.floor===2&&t.id!==S.selectedTable);
  const smRow=fl=>`<div class="move-floor"><i class="fa-solid fa-${fl}"></i> Piso ${fl}</div>
    <div class="move-grid-inner">${DB.getTables().filter(t=>t.floor===fl&&t.id!==S.selectedTable).map(t=>{
      const has=!!DB.getActiveOrder(t.id);
      return `<div class="mt-sm ${has?'occupied':''}" onclick="pickMoveTarget(${t.id},this)"><div class="mts-n">${t.num}</div><div class="mts-s">${has?'Ocupada':'Libre'}</div></div>`;
    }).join('')}</div>`;
  $('move-grid').innerHTML=smRow(1)+smRow(2);
...
function renderInvRows(){
  $('inv-rows').innerHTML=S.tempInv.map((inv,i)=>`
    <div class="inv-row">
      <input class="field-c" placeholder="Descripción (ej: Carne de res)" value="${inv.desc}" oninput="S.tempInv[${i}].desc=this.value">
      <input class="field-c inv-amt" type="number" placeholder="$ Monto" value="${inv.amount}" oninput="S.tempInv[${i}].amount=this.value;recalcInv()">
...
function confirmOpen(){
  ...
  closeModal('modal-open'); toast('¡Local abierto! 🏪 Buena jornada','success'); updateLocalBadges();
...
function openCloseDayModal(){
  ...
  if(active.length){ toast(`${active.length} pedido(s) sin cobrar. Ciérralos antes de terminar la jornada.`,'error'); return; }
  const {sales,cost,profit,paid}=dayStats(day.id);
  const totalInv=day.investments.reduce((s,i)=>s+(i.amount||0),0);
  const netProfit=profit-totalInv;
  $('close-sum').innerHTML=`
    <div class="cds-grid">
      ...
      <div class="cds-box"><div class="cds-l" style="color:var(--text-m)">Costo producción</div><div class="cds-v" style="color:var(--text-m)">${fmtCOP(cost)}</div></div>
      <div class="cds-box ${netProfit>=0?'green':'red'}"><div class="cds-l">Ganancia neta</div><div class="cds-v">${fmtCOP(netProfit)}</div></div>
    </div>
    <div style="background:var(--bg);border-radius:8px;padding:10px 14px;font-size:12px;color:var(--text-m)">
      ${paid} pedido(s) cobrado(s) · Apertura: ${fmtTime(day.openedAt)}
    </div>`;
  $('close-notes').value='';
...
function confirmClose(){
  ...
  closeModal('modal-close-day'); toast('Jornada cerrada 🔒','info'); updateLocalBadges();
...
function renderDashboard(c){
  ...
  const netProfit=ds.profit-totalInv;
  ...
  const prods=DB.getProducts().filter(p=>p.status==='active').length;

  c.innerHTML=`
    <div class="ph"><h2>Dashboard</h2><p>${day?`Jornada abierta · ${fmtTime(day.openedAt)}`:'Sin jornada activa'}</p></div>

    ${!day?`<div class="warn-bar">
      <p><strong>El local está cerrado.</strong> Abre la jornada para comenzar a operar.</p>
      <button class="pill-btn pill-btn--green" onclick="openOpenModal()"><i class="fa-solid fa-store"></i> Abrir Local</button>
    </div>`:''}

    <div class="stats-row">
      <div class="stat-card"><div class="sc-label">Productos Activos</div><div class="sc-val">${prods}</div></div>
      <div class="stat-card"><div class="sc-label">Mesas Ocupadas</div><div class="sc-val">${occ}</div><div class="sc-sub">de 37 totales</div></div>
      <div class="stat-card"><div class="sc-label">Pedidos Activos</div><div class="sc-val">${active}</div></div>
      <div class="stat-card" style="border-color:rgba(240,192,64,.3)"><div class="sc-label" style="color:var(--yellow)">Pago Pendiente</div><div class="sc-val" style="color:var(--yellow)">${pend.length}</div></div>
      ${day?`
      <div class="stat-card"><div class="sc-label">Ventas Hoy</div><div class="sc-val" style="font-size:26px">${fmtCOP(ds.sales)}</div></div>
      <div class="stat-card"><div class="sc-label">Ganancia Neta</div><div class="sc-val" style="font-size:26px;color:${netProfit>=0?'var(--green)':'var(--red)'}">${fmtCOP(netProfit)}</div></div>`:''}
    </div>

    ${pend.length?`<div class="pend-box"><h4><i class="fa-solid fa-triangle-exclamation"></i> Mesas esperando cobro</h4><div class="pend-btns">
      ${pend.map(o=>{const t=DB.getTables().find(x=>x.id===o.tableId);return `<button class="pend-btn" onclick="openPayModal(${o.id})"><i class="fa-solid fa-money-bill-wave"></i> Mesa ${t?.num} P${t?.floor} · ${fmtCOP(orderTotal(o.id))}</button>`;}).join('')}
    </div></div>`:''}
    ...
    <div class="floor-label"><h3><i class="fa-solid fa-building"></i> Piso ${S.floor}</h3></div>
    <div class="tables-grid">
      ${DB.getTables().filter(t=>t.floor===S.floor).map(t=>{
        const o=DB.getActiveOrder(t.id); const total=o?orderTotal(o.id):0;
        return `<div class="table-card ${t.status}" onclick="bossPickTable(${t.id})">
          <div class="tc-icon">${{free:'🟢',occupied:'🍽️',pending:'💛'}[t.status]||'⚪'}</div>
          <div class="tc-num">${t.num}</div>
          <div class="tc-status">${{free:'Libre',occupied:'Ocupada',pending:'Cobro pendiente'}[t.status]||t.status}</div>
          ${o?`<div class="tc-total">${fmtCOP(total)}</div>`:''}
        </div>`;
      }).join('')}
    </div>`;
...
function renderAllOrders(c){
  ...
  const card=o=>{
    const t=DB.getTables().find(x=>x.id===o.tableId);
    const u=DB.getUsers().find(x=>x.id===o.waiterId);
    const items=DB.orderItems(o.id);
    const total=orderTotal(o.id);
    const bmap={active:'badge-green',pending:'badge-yellow',paid:'badge-acc'};
    const lmap={active:'Activo',pending:'Pago Pendiente',paid:'Cobrado'};
    return `<div class="oc">
      <div class="oc-head">
        <h4>Pedido #${o.id} · Mesa ${t?.num||'?'} P${t?.floor||'?'}</h4>
        <div style="display:flex;gap:8px;align-items:center">
          <span class="badge ${bmap[o.status]||'badge-acc'}">${lmap[o.status]||o.status}</span>
          <span style="font-family:'Bebas Neue',sans-serif;font-size:18px;color:var(--acc)">${fmtCOP(total)}</span>
      ...
      <div class="oc-meta">${u?.name||'?'} · ${fmtTime(o.createdAt)}${o.payMethod?' · '+o.payMethod:''}</div>
      <div class="oc-items">${items.map(i=>{const p=DB.getProducts().find(x=>x.id===i.productId);return `<span class="oi-chip ${i.status==='cancelled'?'cancelled':''}">${p?.emoji||''} ${p?.name||'?'} ×${i.qty}</span>`;}).join('')}</div>
    </div>`;
  };
  c.innerHTML=`
    <div class="ph"><h2>Todos los Pedidos</h2></div>
    <h3 style="font-size:22px;color:var(--yellow);margin-bottom:10px"><i class="fa-solid fa-clock"></i> Pago Pendiente (${pend.length})</h3>
    ${pend.length?pend.map(card).join(''):'<p style="color:var(--text-m);font-size:13px;margin-bottom:20px">Ninguno</p>'}
    <h3 style="font-size:22px;color:var(--acc);margin:20px 0 10px"><i class="fa-solid fa-fire"></i> Activos (${active.length})</h3>
    ${active.length?active.map(card).join(''):'<p style="color:var(--text-m);font-size:13px;margin-bottom:20px">Ninguno</p>'}
    <h3 style="font-size:22px;color:var(--green);margin:20px 0 10px"><i class="fa-solid fa-circle-check"></i> Cobrados</h3>
    ${paid.length?paid.map(card).join(''):'<p style="color:var(--text-m);font-size:13px">Ninguno aún</p>'}`;
}
...
function renderMenuView(c){
  ...
  $('sv-grid').innerHTML=DB.getProducts().filter(p=>p.status==='active').map(p=>`
    <div class="pc">
      <div class="pc__img">${p.image?`<img class="prod-img-real" src="${p.image}" alt="${p.name}" loading="lazy">`:`<div class="pc__img-emoji">${p.emoji}</div>`}
        <span class="pc__cat ${getCatClass(p.cat)}">${p.cat}</span></div>
      <div class="pc__body"><div class="pc__name">${p.name}</div><div class="pc__desc">${p.desc||''}</div>
...
function renderProducts(c){
  c.innerHTML=`
    <div class="ph-row">
      <div class="ph"><h2>Productos</h2></div>
      <button class="pill-btn pill-btn--accent" onclick="openProdModal()"><i class="fa-solid fa-plus"></i> Nuevo</button>
    </div>
...
function renderProdTable(){
  ...
  body.innerHTML=DB.getProducts().filter(p=>(!cat||p.cat===cat)&&(!q||p.name.toLowerCase().includes(q))).map(p=>{
    ...
    return `<tr>
      <td>${th}</td>
      <td><strong>${p.name}</strong><br><span style="font-size:11px;color:var(--text-m)">${(p.desc||'').substring(0,50)}…</span></td>
      <td><span class="badge ${getCatClass(p.cat)}">${p.cat}</span></td>
      <td style="font-family:'DM Mono',monospace;font-size:13px;color:var(--yellow)">${fmtCOP(p.cost||0)}</td>
      ...
      <td>
        <div style="display:flex;gap:6px">
          <button class="pill-btn pill-btn--sm" onclick="openProdModal(${p.id})" title="Editar"><i class="fa-solid fa-pen"></i></button>
          <button class="pill-btn pill-btn--sm ${p.status==='active'?'pill-btn--danger':''}" onclick="toggleProdStatus(${p.id})"><i class="fa-solid ${p.status==='active'?'fa-eye-slash':'fa-eye'}"></i></button>
          <button class="pill-btn pill-btn--sm pill-btn--danger" onclick="deleteProd(${p.id})" title="Eliminar"><i class="fa-solid fa-trash"></i></button>
        </div>
      </td>
    </tr>`;
  }).join('');
...
function openProdModal(id){
  ...
  if(id){
    ...
    $('mp-name').value=p.name; $('mp-desc').value=p.desc||''; $('mp-emoji').value=p.emoji;
    $('mp-cat').value=p.cat; $('mp-price').value=p.price; $('mp-cost').value=p.cost||0;
    $('mp-status').value=p.status; $('mp-img').value=p.image||'';
    setProdImgPreview(p.image||null,p.emoji||'🍔');
  } else {
...
function toggleProdStatus(id){
  const p=DB.getProducts().find(x=>x.id===id); if(!p) return;
  DB.upsertProduct({...p,status:p.status==='active'?'inactive':'active'});
  toast(`${p.name} ${p.status==='active'?'desactivado':'activado'}`,p.status==='active'?'info':'success');
  renderProdTable();
}
function deleteProd(id){
  if(!confirm('¿Eliminar este producto? No se puede deshacer.')) return;
  DB.deleteProduct(id); toast('Producto eliminado','error'); renderProdTable();
}
// Image upload for product modal
function setProdImgPreview(img,emoji){
  const prev=$('img-prev'), up=$('img-up');
  if(img){ prev.innerHTML=`<img src="${img}" alt="preview">`; up.classList.add('has-img'); }
  else   { prev.innerHTML=`<span id="img-eph" style="font-size:48px">${emoji}</span>`; up.classList.remove('has-img'); }
}
function loadImg(input){
  ...
  reader.onload=e=>{
    ...
    img.onload=()=>{
      ...
      let nw=w,nh=h;
      if(w>max||h>max){ if(w>h){nh=Math.round(h*max/w);nw=max;}else{nw=Math.round(w*max/h);nh=max;} }
      ...
      $('mp-img').value=b64; setProdImgPreview(b64,'🍔');
...
function clearImg(){ $('mp-img').value=''; setProdImgPreview(null,$('mp-emoji').value||'🍔'); toast('Imagen eliminada','info'); }
...
function renderUsers(c){
  ...
  c.innerHTML=`
    <div class="ph-row">
      <div class="ph"><h2>Usuarios</h2><p>Credenciales de acceso al sistema</p></div>
    ...
    <div style="overflow-x:auto">
      <table class="pt">
        ...
        <tbody>
          ${users.map(u=>`<tr>
            <td><span style="font-family:'DM Mono',monospace;font-size:13px;color:var(--acc)">${u.username}</span></td>
            <td><strong>${u.name}</strong></td>
            <td><span class="badge ${u.role==='boss'?'badge-acc':'badge-blue'}">${u.role==='boss'?'Jefe':'Mesero'}</span></td>
            <td><span class="badge ${u.active?'badge-green':'badge-red'}">${u.active?'Activo':'Inactivo'}</span></td>
            <td><div style="display:flex;gap:6px">
              <button class="pill-btn pill-btn--sm" onclick="editUserPass(${u.id})"><i class="fa-solid fa-key"></i> Cambiar contraseña</button>
              ${u.id!==S.user.id?`<button class="pill-btn pill-btn--sm ${u.active?'pill-btn--danger':''}" onclick="toggleUser(${u.id})">${u.active?'Desactivar':'Activar'}</button>`:''}
            </div></td>
          </tr>`).join('')}
        </tbody>
      </table>
...
function editUserPass(uid){
  const np=prompt('Nueva contraseña para este usuario:'); if(!np) return;
  const arr=DB.getUsers(); const i=arr.findIndex(x=>x.id===uid); if(i<0) return;
  arr[i].password=np; DB.saveUsers(arr); toast('Contraseña actualizada ✅','success'); renderUsers($('st-content'));
}
function toggleUser(uid){
  const arr=DB.getUsers(); const i=arr.findIndex(x=>x.id===uid); if(i<0) return;
  arr[i].active=!arr[i].active; DB.saveUsers(arr);
  toast(`Usuario ${arr[i].active?'activado':'desactivado'}`,arr[i].active?'success':'info');
  renderUsers($('st-content'));
}
function promptNewUser(){
  ...
  const role=confirm('¿Es Jefe/Admin?\n(Aceptar = Jefe · Cancelar = Mesero)')?'boss':'waiter';
  const arr=DB.getUsers(); const id=DB.seq('user');
  arr.push({id,username,password,role,name,active:true,createdAt:new Date().toISOString()});
  DB.saveUsers(arr); toast(`Usuario ${name} creado ✅`,'success'); renderUsers($('st-content'));
}
...
function renderReports(c){
  ...
  const card=day=>{
    ...
    const orders=DB.getOrders().filter(o=>o.dayId===day.id&&o.status==='paid');
    const isOpen=day.status==='open';
    return `<div class="rcard">
      <div class="rcard-head">
        <div><h4>${fmtDate(day.openedAt)}</h4>
          <span style="font-size:12px;color:var(--text-m)">Apertura: ${fmtTime(day.openedAt)}${day.closedAt?` · Cierre: ${fmtTime(day.closedAt)}`:''}</span>
        </div>
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          <span class="badge ${isOpen?'badge-green':'badge-acc'}">${isOpen?'🟢 Abierta':'🔒 Cerrada'}</span>
          ${isOpen?`<button class="pill-btn pill-btn--sm pill-btn--danger" onclick="openCloseDayModal()"><i class="fa-solid fa-lock"></i> Cerrar</button>`:''}
        </div>
      </div>
      <div class="rcard-metrics">
        ...
        <div class="rm"><div class="rm-l">Gan. neta</div><div class="rm-v" style="color:${net>=0?'var(--green)':'var(--red)'}; font-size:26px">${fmtCOP(net)}</div></div>
        <div class="rm"><div class="rm-l">Pedidos</div><div class="rm-v">${paid}</div></div>
      </div>
      ${day.investments.length?`<div class="rcard-inv"><h5><i class="fa-solid fa-boxes-stacking"></i> Inversiones</h5>
        ${day.investments.map(inv=>`<div class="ri"><span>${inv.desc}</span><span style="color:var(--yellow);font-family:'DM Mono',monospace">${fmtCOP(inv.amount)}</span></div>`).join('')}
      </div>`:''}
      ${orders.length?`<div class="rcard-orders"><h5><i class="fa-solid fa-list-check"></i> Pedidos cobrados (${orders.length})</h5>
        <div class="ro-chips">${orders.map(o=>{const t=DB.getTables().find(x=>x.id===o.tableId);return `<span class="ro-chip">Mesa ${t?.num||'?'} P${t?.floor||'?'} · ${fmtCOP(orderTotal(o.id))} · ${o.payMethod||'?'}</span>`;}).join('')}</div>
      </div>`:''}
      ${day.openNotes||day.closeNotes?`<div style="padding:10px 20px;border-top:1px solid var(--border);font-size:12px;color:var(--text-m)">
  ...
  c.innerHTML=`
    <div class="ph-row">
      ...
      ${!DB.getCurrentDay()?`<button class="pill-btn pill-btn--green" onclick="openOpenModal()"><i class="fa-solid fa-store"></i> Nueva Jornada</button>`:''}
    </div>
    ${days.length?`<div class="report-list">${days.map(card).join('')}</div>`:`
